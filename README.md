...
- 记下远程地址，例如 `https://github.com/<你的用户名>/XSAN_GOGUI.git`。

### 2️⃣ 本地初始化并首次推送

在 **`XSAN_GOGUI` 根目录**执行（按你的邮箱/用户名修改）：
...
git commit -m "Initial commit: XSAN Wails PE scanner client"

git remote add origin https://github.com/<你的用户名>/XSAN_GOGUI.git
...
```powershell
git remote add origin git@github.com:<你的用户名>/XSAN_GOGUI.git
git push -u origin main
```
...
Made with 💙 for graduation project **XSAN_GOGUI**
